...
def hollys_store(result):
    for page in range(1,59):
        ...
        for store in tag_tbody.find_all('tr'):
            ...
            sql = "insert into hollysTBL values(null,'" + str(store_name) + "','" + str(store_sido) + "','"
            sql = sql + str(store_address) + "','" + str(store_phone) + "')"
            cur.execute(sql)
...
def main():
    ...
    hollys_tbl = pd.DataFrame(result, columns=('매장명', '시도구', '상세주소','전화번호'))
